...
const ValueAnalysis = ({ originalListing, relatedListings }) => {
  const calculations = useMemo(() => {
    ...
    const marketPrices = relatedListings.map(listing => parseFloat(listing.price.replace(/[$,]/g, '')));
    const avgMarketPrice = marketPrices.reduce((sum, price) => sum + price, 0) / relatedListings.length;
    
    // Calculate standard deviation for market prices
    const priceVariance = marketPrices.reduce((sum, price) => sum + Math.pow(price - avgMarketPrice, 2), 0) / marketPrices.length;
    const priceStdDev = Math.sqrt(priceVariance);

    // Calculate percentile of current price in the market
    const sortedPrices = [...marketPrices].sort((a, b) => a - b);
    const pricePercentile = (sortedPrices.findIndex(price => price >= originalPrice) / sortedPrices.length) * 100;
    
    // Calculate price deviation as a percentage
    const priceDeviation = ((originalPrice - avgMarketPrice) / avgMarketPrice) * 100;
    
    // Determine market demand factors
    const avgLikes = relatedListings.reduce((sum, listing) => sum + parseInt(listing.likesCount || 0, 10), 0) / relatedListings.length;
    ...
    const marketAlignmentFactor = priceDeviation > 15 ? 0.15 : priceDeviation > 0 ? priceDeviation / 100 : 0;
    ...
    const adjustmentPercentage = (marketAlignmentFactor + ageFactor + demandFactor) * 100;
    const suggestedReduction = originalPrice * (marketAlignmentFactor + ageFactor + demandFactor);
    
    // Suggested lowball price using dynamic factors
    const marketVolatility = priceStdDev / avgMarketPrice;
    const lowballFactor = 0.15 + (marketVolatility * 0.5) + (listingAge > 21 ? 0.1 : 0);
  ...
  const getSellerRecommendations = () => {
    ...
    if (calculations.listingAge > 30) {
        recommendations.push({
            priority: 'high',
            action: 'You should refresh your listing by updating the photos or enhancing the description to reinvigorate interest.',
            reasoning: 'Your listing has been active for over a month with limited buyer engagement, suggesting it may need a visual or informational update to attract attention.'
        });
    } else if (calculations.listingAge > 14 && parseFloat(calculations.demandRatio) < 0.8) {
        recommendations.push({
            priority: 'medium',
            action: 'Consider updating your listing details or making a slight price adjustment to improve its performance.',
            reasoning: 'Engagement is currently lower than average for similar listings after two weeks, indicating that minor adjustments could increase interest.'
        });
    }
    ...
    if (parseFloat(calculations.demandRatio) > 1.5) {
        recommendations.push({
            priority: 'low',
            action: 'You might want to experiment with a slightly higher price to gauge buyer response.',
            reasoning: 'Your listing is attracting significantly more interest compared to similar items, suggesting there could be room for a price increase.'
        });
    } else if (parseFloat(calculations.demandRatio) < 0.6) {
        recommendations.push({
            priority: 'high',
            action: 'Focus on improving your listing’s visibility by enhancing the keywords or uploading higher-quality photos.',
            reasoning: 'Your listing is receiving considerably less interest compared to similar items, indicating it may not be reaching potential buyers effectively.'
        });
    }
...
const getBuyerRecommendations = () => {
    ...
    if (parseFloat(calculations.priceDeviation) > 25) {
        ...
        recommendations.push({
            priority: 'medium',
            action: `A reasonable counteroffer would be approximately $${(parseFloat(calculations.originalPrice) * 0.9).toFixed(2)}, reflecting a fair market value adjustment.`,
            reasoning: 'Although the price is slightly above market average, it is not excessively so, allowing room for a fair negotiation.'
        });
    } else if (parseFloat(calculations.priceDeviation) < -10) {
        recommendations.push({
            priority: 'high',
            action: 'You should act quickly to secure this deal, as the listing is priced below the market average and represents good value.',
            reasoning: `The item is priced approximately ${Math.abs(parseFloat(calculations.priceDeviation))}% below average, making it an attractive purchase opportunity.`
        });
    }

    // Listing age leverage
    if (calculations.listingAge > 21 && parseFloat(calculations.priceDeviation) > 0) {
        recommendations.push({
            priority: 'high',
            action: 'When making an offer, mention that the listing has been active for over three weeks, as this may encourage the seller to negotiate more flexibly.',
            reasoning: 'Listings that have been on the market for an extended period often indicate a seller’s increasing willingness to lower their price.'
        });
    }
    ...
    if (parseFloat(calculations.marketVolatility) > 25) {
        recommendations.push({
            priority: 'medium',
            action: 'Be prepared to negotiate assertively, as prices within this category exhibit substantial variation.',
        ...
        recommendations.push({
            priority: 'low',
            action: 'Understand that prices in this category tend to be relatively fixed, limiting the potential for negotiation.',
            reasoning: 'Low market volatility suggests that sellers are generally consistent with their pricing, reducing the likelihood of large discounts.'
        });
    }
    ...
    if (parseFloat(calculations.demandRatio) > 1.3) {
        recommendations.push({
            priority: 'high',
            action: 'If you are genuinely interested in this item, avoid prolonged negotiation, as it is likely to sell quickly.',
            reasoning: 'This listing is receiving 30% more interest than comparable items, increasing the risk of it being purchased by another buyer.'
        });
    }
        ...
        <div className="metric">
          ...
          <p>
            {parseFloat(calculations.priceDeviation) > 0 
              ...
              : `Priced ${Math.abs(parseFloat(calculations.priceDeviation))}% below market average`}
          </p>
        </div>
        ...
        <div className="metric">
          ...
          <p>
            {parseFloat(calculations.demandRatio) > 1 
              ? `Generating ${((parseFloat(calculations.demandRatio) - 1) * 100).toFixed(0)}% more interest than average`
              : `Generating ${((1 - parseFloat(calculations.demandRatio)) * 100).toFixed(0)}% less interest than average`}
          </p>
        </div>
      </div>
          ...
          {sellerRecommendations.length > 0 ? (
            <ul className="dynamic-recommendations">
              {sellerRecommendations.map((rec, index) => (
                <li key={`seller-${index}`} className={`priority-${rec.priority}`}>
          ...
          {buyerRecommendations.length > 0 ? (
            <ul className="dynamic-recommendations">
              {buyerRecommendations.map((rec, index) => (
                <li key={`buyer-${index}`} className={`priority-${rec.priority}`}>
                  <div className="recommendation-action">{rec.action}</div>
                  <div className="recommendation-reasoning">{rec.reasoning}</div>
                </li>
              ))}
            ...
            <p>This listing appears to be fairly priced relative to the market.</p>
          )}
        </div>
      </div>
    </div>
