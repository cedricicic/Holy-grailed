...
const handlePopups = async (page) => {
  try {
    await page.waitForSelector("button:has-text('Accept'), .cookie-notice button", { timeout: 1000 });
    await page.evaluate(() => {
      document.querySelectorAll("button:has-text('Accept'), .cookie-notice button").forEach((btn) => btn.click());
    });
  } catch (e) {
